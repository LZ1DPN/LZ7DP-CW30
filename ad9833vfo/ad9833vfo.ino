...
//#define ANALOG_KEYER (A1)  // KEYER input - for analog straight key - like Farhan's minima style.
...
delay(50); 
  ...
  delay(50);
  AD9833setFrequency((rx+freqIF+rxRIT-rxof), SQUARE);                  // Set the frequency and Sine Wave output
//  Serial.println(rx+freqIF+rxRIT-rxof);
  ...
  sei();
    ...
    AD9833setFrequency((rx+freqIF+rxRIT-rxof), SQUARE);     // Set AD9833 to frequency and selected wave type.
    delay(50);
}
...
void checkCW(){
  ...
  if ((keyDown == 1) && (keyerON == HIGH)){
     keyDown = 0;
     inTx = 0; 
     digitalWrite(CW_KEY, LOW);  // stop the side-tone
     digitalWrite(TX_RX, LOW);
	   AD9833setFrequency((rx+freqIF+rxRIT-rxof), SQUARE);     // Set AD9833 to frequency and selected wave type.
...
//old keyer
  ...
  void checkCW(){
  pinMode(TX_RX, OUTPUT);
  if (keyDown == 0 && analogRead(DIG_KEYER) < 50){
    //switch to transmit mode if we are not already in it
    if (inTx == 0){
      //put the TX_RX line to transmit
  ...
  //if we have a keyup
  if (keyDown == 1 && analogRead(DIG_KEYER) > 150){
     keyDown = 0;
     inTx = 0; 
